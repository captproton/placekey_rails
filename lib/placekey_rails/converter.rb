...
module PlacekeyRails
  module Converter
    ...
    def geo_to_placekey(lat, long)
      h3_index = H3Adapter.lat_lng_to_cell(lat, long, PlacekeyRails::RESOLUTION)
      encode_h3_int(h3_index)
    end

    def placekey_to_geo(placekey)
      h3_index = placekey_to_h3_int(placekey)
      H3Adapter.cell_to_lat_lng(h3_index)
    end

    def h3_to_placekey(h3_string)
      h3_int = H3Adapter.string_to_h3(h3_string)
    ...
    def shorten_h3_integer(h3_integer)
      if h3_integer.is_a?(String)
        h3_integer = H3Adapter.string_to_h3(h3_integer)
      end
